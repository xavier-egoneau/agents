...
def _cwd(ctx: RunContext[Any], raw: str) -> Path:
    ...
    target = (ctx.deps.workspace / candidate).resolve() if not candidate.is_absolute() else candidate.resolve()
...
async def process_status(
    ...
    ports = sorted({int(value) for value in re.findall(
        r"(?:localhost|127\.0\.0\.1|0\.0\.0\.0):(\d{2,5})", output
...
class ProcessModule:
    def toolsets(self):
        return [FunctionToolset(tools=[
            command_run, process_start, process_status, process_output, process_stop
        ])]
