...
def test_filesystem_permission_matrix(tmp_path: Path) -> None:
    ...
    assert decide(
        tmp_path, "read", [ToolRisk.READ], SecurityMode.SAFE, "existing.txt"
    ).verdict == GuardianVerdict.ALLOW
    assert decide(
        tmp_path, "write", [ToolRisk.WRITE], SecurityMode.SAFE, "new.txt"
    ).verdict == GuardianVerdict.ASK
    assert decide(
        tmp_path, "write", [ToolRisk.WRITE], SecurityMode.LIMITED, "new.txt"
    ).verdict == GuardianVerdict.ALLOW
    assert decide(
        tmp_path, "write", [ToolRisk.WRITE], SecurityMode.LIMITED, "existing.txt"
    ).verdict == GuardianVerdict.ASK
    assert decide(
        tmp_path, "write", [ToolRisk.WRITE], SecurityMode.POWER, "existing.txt"
    ).verdict == GuardianVerdict.ALLOW
    for mode in SecurityMode:
        assert decide(
            tmp_path, "delete", [ToolRisk.DESTRUCTIVE], mode, "existing.txt"
        ).verdict == GuardianVerdict.ASK
...
def test_outside_and_protected_paths(tmp_path: Path) -> None:
    ...
    assert decide(
        tmp_path, "read", [ToolRisk.READ], SecurityMode.POWER, str(outside)
    ).verdict == GuardianVerdict.ASK
    assert decide(
        tmp_path, "read", [ToolRisk.READ], SecurityMode.POWER, ".ssh/id_rsa"
    ).verdict == GuardianVerdict.DENY
    outside.write_text("secret")
    (tmp_path / "link").symlink_to(outside)
    assert decide(
        tmp_path, "read", [ToolRisk.READ], SecurityMode.POWER, "link"
    ).verdict == GuardianVerdict.DENY
...
def test_missing_justification_is_denied(tmp_path: Path) -> None:
    decision = review_tool_call(
        ...
        arguments={"path": "file.txt"}, risks=[ToolRisk.READ],
...
def test_private_web_target_always_requires_approval(tmp_path: Path) -> None:
    decision = review_tool_call(
        tool_name="web", tool_call_id="call-web", agent_id="main",
        arguments={
            "action": "scrape", "url": "http://127.0.0.1:8080/private",
            "justification": "Inspect a local service.",
        },
        risks=[ToolRisk.NETWORK, ToolRisk.EXTERNAL],
        mode=SecurityMode.POWER, workspace=tmp_path,
    )
